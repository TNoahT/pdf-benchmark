...
OUTPUT_CSV = "./Data/results/Tabula/tabula_output.csv"
...
def extract_tables_from_pdf(pdf_path) :
    try : 
        ...
        if tables is None :
            ...
            return []
        return [' '.join(df.fillna('').astype(str).values.flatten()) for df in tables]
    except Exception as e :
        print(f"[ERROR] Tabula extraction failed for {pdf_path}: {e}")
        return []
...
def compute_metrics_all(gt_text, extracted_tables, pdf_name, page_num, tool_name="Tabula", label="table"):
...
def main():
    ...
    for pdf_path in pdf_files:
        ...
        for txt_path in matching_txts:
            ...
            tables = tabula.read_pdf(pdf_path, pages=page_num+1, multiple_tables=True, stream=True)
            if not tables:
                continue

            extracted = [' '.join(df.fillna("").astype(str).values.flatten()) for df in tables]
